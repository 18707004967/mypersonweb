	...
	<div id="singerlist">
		<ul class="list_rank">
			<li v-for="(value , index) in list" :key="index">
				<router-link :to="{path:'/singerListInner',
					query:{
...
export default {
	...
	mounted(){
		...
		this.$axios.get('/api/singer/class&json=true')
		.then(res=>{
...
<style scoped>
#singerlist{ padding: 1.8rem 0 0 0 ; background: #f4f4f4; }
.list_rank{ width: 90%; margin: 0 auto;}
.list_rank li{ width: 100%; margin: 0.10rem auto; height: 2.0rem; background: #f1f1f1; border-radius: 0.20rem; border-bottom: #999 solid 0.03rem; }
.list_rank li>a{ display: flex; width: 100%; height: 100%;}
